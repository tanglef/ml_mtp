...
\begin{frame}{Ridge estimator}{How to compute it efficiently}
    ...
    Problem : $\lambda$ is a tuning parameter $\Rightarrow$ computing many solution of $\hat \beta_{\lambda}$
    % \begin{itemize}
    %     \item $\lambda$ is a tuning parameter $\Rightarrow$ computing many solution of $\hat \beta_{\lambda}$
    % \end{itemize}
   \begin{onlyenv}<2->
        \begin{block}{Use ONE SVD ($X = U D V^\top$) to compute many estimation}
    ...
    \end{onlyenv}
    \begin{onlyenv}<3->
        \begin{block}{Use ONE SVD ($X = U D V^\top$) to compute many prediction}
        % Use SVD (singular value decomposition) to compute efficiently many solution :
        %\begin{align*}
...
\begin{frame}{Ridge estimator}{LOO CV}
    ...
    \begin{onlyenv}<3->
        \begin{block}{}
        $$ R^\lambda
        =  X(X^\top X + \lambda \mathrm{Id})X^\top
...
\begin{frame}{Ridge estimator}{Bias and Variance}
    ...
    Suppose that data arise from linear model,  with i.i.d zero-mean errors $\varepsilon_i$
    $$ y_i = x_i^\top \beta + \varepsilon_i, \quad i=1, \dots, n$$
    Then, the Ridge estimate $\hat \beta_{\lambda}$ are biaised estimate of $\beta$.
    \begin{onlyenv}<2->
        \begin{block}{Bias}
            If the $x_i$ are assumed fixed, $n > p$ and $X$ has full column rank, we get :
            $$ Biais(\hat \beta_\lambda) = \sum_{j=1}^p v_j \frac{\lambda}{d_j^2 + \lambda} \langle v_j \vert \beta \rangle $$
        \end{block}
        Smaller the $j^{th}$ singular values is, bigger is the shrinkage associated.
    \end{onlyenv}
    % Let us note :
    % \begin{itemize}
    %     \item $\hat \beta_\lambda^{(-i)}$ the estimated coefficients without using the pair $(x_i, y_i)$.
